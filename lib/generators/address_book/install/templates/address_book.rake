if defined? AddressBook::Engine
  namespace "address_book" do
    ...
    desc "Import address_book's assets and new db migrations"
    ...
    namespace :db do #
      ...
      desc "Import address_book's new db migrations"
      task :migrate do
        require 'rails/generators/active_record'
        db_migrate = File.join Rails.root, *%w{db migrate}

        # Consider the set of engine migrations, in chronological order.
        migrations = Dir[File.join AddressBook::Engine.root, *%w{db migrate [0-9]*_*.rb}]
        migrations.sort!
        ...
        last_migration_in_schema = migrations.reverse.detect do |migration|
          ...
          Dir[File.join(db_migrate, "[0-9]*_address_book_schema_after_#{name}.rb")].any?
        end
        ...
        migrations.each do |old_migration|
          ...
          new_name = "address_book_#{old_name}"
      ...
      desc "Import address_book's schema as a db migration"
      task :schema do
        schema = File.join AddressBook::Engine.root, *%w{db schema.rb}
        if File.exist? schema
          ...
          migration_name = if latest_migration
            ...
            "address_book_schema_after_#{latest_migration_name}"
          else
            "address_book_schema"
          end
...
EOF
      ...
      task "migrate:seed" do
        migration_name = "address_book_seed"
        if File.exist? File.join(AddressBook::Engine.root, 'db', 'seeds.rb')
          host_migration migration_name do
<<-EOF
class #{migration_name.camelize} < ActiveRecord::Migration
  def self.up
    load File.join(AddressBook::Engine.root, 'db', 'seeds.rb')
  end
end
EOF
          end
      ...
      desc "Load address_book's seed data"
      task :seed => :environment do
        seed_file = File.join(AddressBook::Engine.root, 'db', 'seeds.rb')
        load(seed_file) if File.exist?(seed_file)
      end

    end

    desc "Link (or copy) address_book's static assets"
    task :assets, [:copy] => :environment do |t, args|
      engine_asset_path = File.join(
        Rails.application.paths.public.to_a.first,
        AddressBook::Engine::ASSET_PREFIX)

      rm_rf engine_asset_path
      host_asset_path = AddressBook::Engine.paths.public.to_a.first
